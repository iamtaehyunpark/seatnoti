  ...
  for (const [apiPath, items] of Object.entries(uniqueRequests)) {
    try {
      ...
      for (const item of items) {
        // --- NEW: MUTE CHECK via notificationMode ---
        // Backward compatibility helper (in case background runs before popup migration)
        const mode = item.notificationMode || (item.isMuted ? 'NONE' : (item.isWaitlistMuted ? 'OPEN_ONLY' : 'ALL'));
        ...
        if (livePkg) {
          ...
          if (liveSection) {
            ...
            if (status === "OPEN" || status === "WAITLISTED") {
              // Final Check against Mode
              if (status === "OPEN" && mode === 'WAITLIST_ONLY') {
                // Do nothing
              } else if (status === "WAITLISTED" && mode === 'OPEN_ONLY') {
                // Do nothing
              } else {
